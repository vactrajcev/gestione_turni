...
# --- CONFIGURAZIONE ---
st.set_page_config(page_title="Gestione Turni V64.2 - Safe Night", layout="wide")
st.title("🗓️ Sistema Gestione Turni - V64.2")
st.markdown("### 🛡️ Protocollo Copertura Totale Notti + Incompatibilità Smart")
...
with col_op:
    ...
    op_df = st.data_editor(pd.DataFrame(st.session_state.operatori), num_rows="dynamic", key="op_v64_2",
                           column_config={
...
with col_inc:
    ...
    inc_df = st.data_editor(pd.DataFrame(columns=["Op A", "Op B"]), num_rows="dynamic", key="inc_v64_2",
                            column_config={
...
with col_ass:
    ...
    ass_df = st.data_editor(pd.DataFrame(columns=["Operatore", "Dal", "Al"]), num_rows="dynamic", key="ass_v64_2",
                            column_config={"Operatore": st.column_config.SelectboxColumn("Op", options=lista_nomi)})
with col_pref:
    st.subheader("⭐ Preferenze")
    pref_df = st.data_editor(pd.DataFrame(columns=["Operatore", "Giorno", "Turno"]), num_rows="dynamic", key="pref_v64_2",
                             column_config={
...
def genera_piano(anno, mese):
        ...
        for t_tipo, qta in [("N", 1), ("M", 2), ("P", 2)]:
            while res[col].tolist().count(t_tipo) < qta:
                ...
                if not cand_f and t_tipo == "N":
                    # Se non c'è nessuno, cerchiamo chiunque abbia ancora spazio nelle notti e non sia in ferie/smonto
                ...
                if t_tipo == "N":
                    # Priorità assoluta a chi ha meno notti rispetto al proprio limite
    ...
    for n in nomi:
        ...
        an_rows.append({"Operatore": n, "Notti": n_c, "Max": lim_n[n], "Ore": o_e, "Target": targ_ore[n], "Sat%": round(o_e/targ_ore[n]*100, 1) if targ_ore[n]>0 else 0})
...
if st.button("🚀 GENERA PIANO V64.2"):
    ...
    st.subheader("📊 Analisi Equità e Bilanciamento")
    st.table(an)
    st.download_button("📥 Scarica Excel", data=to_excel(tab, an), file_name=f"Turni_V64_2.xlsx")
